...
async def test_save_stgcn_history(mock_hot_storage):
    ...
    history_buffer = [(85, arr1), (86, arr2)]
    ...
    # Kiểm tra frames lưu dưới dạng list dict
    assert isinstance(update_doc["$set"]["frames"][0], dict)
    assert update_doc["$set"]["frames"][0]["slot"] == 85
    assert isinstance(update_doc["$set"]["frames"][0]["data"], Binary)
...
async def test_load_stgcn_history(mock_hot_storage):
    ...
    arr2 = np.array([4.0, 5.0, 6.0], dtype=np.float32)
    binary_frames = [
        {"slot": 85, "data": Binary(arr1.tobytes())},
        {"slot": 86, "data": Binary(arr2.tobytes())}
    ]
    ...
    assert isinstance(restored_history[0], tuple)
    assert restored_history[0][0] == 85
    assert isinstance(restored_history[0][1], np.ndarray)
    assert restored_history[0][1].dtype == np.float32
    
    # Kiểm tra tính vẹn toàn dữ liệu (Data Integrity)
    assert np.array_equal(restored_history[0][1], arr1)
    assert np.array_equal(restored_history[1][1], arr2)
